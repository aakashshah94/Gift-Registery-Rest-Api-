...
$(document).ready(function() {
	    ...
	    $.each(${jsonstring} ,function(index,product)
	              {
	    	         if(product.assignment==0)
	    	        	 {
	    	            $('<tr id="'+product.productId+'"><td>'+product.productName+'</td>'
	    	            	+'<td>'+product.productDescription+'</td>'
	    	            	+'<td>'+product.price+'</td>'
	    	            	+'<td>'+product.comapny+'</td>'
	    	            	+'<td><button onclick="add(this)">Assign to ME</tr>').appendTo($("#example"));
	    	        	 }
	    	         else
	    	        	 {
	    	        	 $('<tr><td>'+product.productName+'</td>'
	 	    	            	...
	 	    	            	+'<td>'+product.price+'</td>'
	 	    	            	+'<td>'+product.comapny+'</td>'
	 	    	            	+'<td>'+product.assignedby+'</tr>').appendTo($("#example"));
...
{ 
	...
	$.post("HandleShared", {service:"assignme",data:thisid,rname:ta} , function(responseJson) {
        alert("success");
	});
}
				...
				<div class="navbar-inner main-menu">				
					<a href="index.jsp" class="logo pull-left"><img src="themes/images//giftItlogo.png" class="site_logo" alt=""></a>
						...
						<ul>
							...
							<li><a href="viewshared.jsp">View Shared Registries</a></li>							
    ...
    <caption id="caption"> ${regname} </caption>
