...
<script type="text/javascript">
	 $(document).ready(function() { // When HTML DOM "click" event is invoked on element with ID "somebutton", execute the following function...
		  $.get("HandlePublic", function(responseJson) {   // Execute Ajax GET request on URL of "someservlet" and execute the following function with Ajax response text...

			  $.each(responseJson,function(index,product)
		               {
				        $('<form id="my_form" method="post" action="regs"><input type="hidden" name="service" value="regDetail">'+product+'<input type="hidden" name="regname" value="'+product+'"/> <button type="submit">View Directory</button></form>').appendTo($("#allregs"));
		               });  
